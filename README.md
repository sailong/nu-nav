...
- **🔍 智能搜索**：
  - **站内搜索**：实时过滤本地书签。
  - **多引擎切换**：支持 Google、Bing、百度等主流搜索引擎。
  - **自定义引擎**：支持在后台灵活添加、修改或删除搜索源及其占位符。
- **🛠️ 强大后台**：
  - **分类管理**：支持自定义分类名称与排序。
  - **标签管理**：轻松管理 Logo、链接、描述，支持 Logo 实时预览。
  - **外观设置**：一键更换系统标题、Favicon 浏览器图标及前端背景图。
...
本项目已合并为单体容器，内置 Nginx (前端) 和 Node.js (后端)，支持 AMD64 和 ARM64。
...
如果您更喜欢使用 `docker-compose` 进行管理，可以使用项目根目录下的 `docker-compose.yml`：
...
- **端口**: 容器暴露 `80` 端口。
- **数据**: 数据库文件和自动生成的 JWT 密钥保存在挂载的 `data` 目录中，重启容器不会丢失登录状态。
